"""Every log line carries the call it belongs to (roma.logging_setup).

Until this existed, exactly two lines in the system named a call: the stream-start line and
the spend-ledger row. Everything from `roma.controller`, `roma.guardrails`, `pretts`,
...
from roma.logging_setup import CallSidFilter, current_call_sid


def _record():
    return logging.LogRecord("roma.controller", logging.INFO, __file__, 1, "hi", None, None)
...
def test_a_record_outside_a_call_still_formats():
    ...
    assert fmt.format(rec) == "roma.controller [-] hi"
