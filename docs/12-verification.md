...
| **G0** | Consent + DND + calling window gate the dialer; fail-safe is BLOCK. | `tests/domain/calls/`. Confirm `place_call` still enforces the window — `--ignore-window` in `place_test_call.py` must substitute a `now` for the check and **not** touch `domain/calls/window.py`. | verified offline |
...
| **2** | The VAD is a pipeline **stage**, not a transport param. | `tests/realtime/test_vad_stage.py` (it pins `"vad_analyzer" not in FastAPIWebsocketParams.model_fields`, so a future pipecat adding it back fails loudly). Live: teardown must show `vad_starts=N vad_stops=N` with N>0. | verified live |
